...
// default is mean whatever is the default languange in the browser
// here is function for days
class Day {
...
//here is the function for the Month
class Month {
  constructor(date = null, lang = "default") {
...
class DatePicker extends HTMLElement {
  ...
  // here web component function  if we update in the browser will reflect
  ...
  //here is the weekday set (start from Sun -Sat)
  getWeekDaysElementStrings() {
    return this.calendar.weekDays
      .map((weekDay) => `<span>${weekDay.substring(0, 3)}</span>`)
      .join("");
  }
  // here we start from the first day of the Month then create a for loop
  getMonthDaysGrid() {
